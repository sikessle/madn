...
final class MovementController extends Observable {
	...
	public boolean rollDice(Player player) {
		...
		if (isAllowedToThrowDice(player)) {
			status = "Dice Thrown: " + dice.rollDice(player) + ".";
		} else {
			status = "You are not allowed to roll the dice.";
		}
	...
	public boolean moveFigure(Player player, char figureLetter) {
		if (dice.getLastThrower() != player || dice.getThrowsCount() == 0) {
			status = "You should roll the dice first!";
			notifyObservers();
			return false;
		}
		...
		if (figure == null) {
			status = "This figure ist not yours!";
			notifyObservers();
			return false;
		}
		...
		if (figure.isFinished()) {
			status = "Figure is already finished!";
			notifyObservers();
			return false;
		}
		...
		if (!figureCanMove(figure, diceNum)) {
			status = "This figure cannot move!";
			notifyObservers();
			return false;
		}
		...
		if (figureCanLeaveHome(figure, diceNum)) {
			...
			if (kickedAwayFigure != null) {
				kicked = " and has kicked away " + kickedAwayFigure.getLetter();
			}
			status = "Figure " + figure.getLetter() + " left his home" + kicked;
		...
		if (figureCanMoveInFinishArea(figure, diceNum)) {
			...
			status = "Figure " + figure.getLetter() + " has movedt!";
			notifyObservers();
			return true;
		}
		...
		if (figureCanMoveInPublic(figure, diceNum)) {
			...
			if (entersFinishField(diceNum, finishEntryIndex,
				...
				if (kickedFigure != null) {
					kicked = " and kicked away " + kickedFigure.getLetter();
				}
			}
			status = "Figure " + figure.getLetter() + " has moved" + kicked;
			notifyObservers();
			return true;
		}
